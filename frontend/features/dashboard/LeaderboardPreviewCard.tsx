...
export default async function DashboardPage() {
  ...
  const [activeSession, todaysGoal, currentFocus, weeklyTrends, rank] = await Promise.all([
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-5 rounded-3xl border border-[#F0E6D6] bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
            <div className="flex items-center justify-between">
              ...
              <Link href="/progress" className="flex items-center gap-1 text-sm font-semibold text-[#4C3AA0] dark:text-indigo-400">
                View Detailed Progress <ArrowRight size={14} />
              </Link>
            </div>
            <div className="mt-5 grid grid-cols-3 gap-4 text-center">
              <div>
                <div className="text-2xl font-extrabold text-[#2E6B1B]" style={{ fontFamily: "var(--font-fredoka), sans-serif" }}>{weeklyTrends.solvedThisWeek}</div>
                <div className="text-xs text-[#6B5D4F] dark:text-neutral-500">Solved</div>
              </div>
              <div>
                <div className={`text-2xl font-extrabold ${(weeklyTrends.ratingChangeThisWeek ?? 0) >= 0 ? "text-[#4C3AA0]" : "text-[#D9502F]"}`} style={{ fontFamily: "var(--font-fredoka), sans-serif" }}>
                  {weeklyTrends.ratingChangeThisWeek !== null ? `${weeklyTrends.ratingChangeThisWeek >= 0 ? "+" : ""}${weeklyTrends.ratingChangeThisWeek}` : "—"}
                </div>
                <div className="text-xs text-[#6B5D4F] dark:text-neutral-500">Rating Change</div>
              </div>
              <div>
                <div className="text-2xl font-extrabold text-[#FF6B4A]" style={{ fontFamily: "var(--font-fredoka), sans-serif" }}>{weeklyTrends.attemptsThisWeek}</div>
