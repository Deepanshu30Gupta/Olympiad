...
export default async function PracticePage({
  ...
  if (!clerkUser) {
    return (
      <div className="min-h-screen bg-[#FFFBF2] p-8 text-[#2B2118] dark:bg-neutral-950 dark:text-neutral-300">
        Not signed in.
      </div>
    );
  }
    ...
    if ("offerRetry" in result && result.offerRetry) {
      return (
        <EmptyStateShell>
          ...
          <h1 className="mt-4 text-xl font-semibold text-[#2B2118] dark:text-neutral-100">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link
              ...
              className="rounded-lg border border-[#F0E6D6] px-4 py-2 text-sm text-[#2B2118] hover:border-[#FF6B4A]/50 dark:border-neutral-800 dark:text-neutral-300 dark:hover:border-neutral-700"
            >
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF2] text-[#2B2118] dark:bg-neutral-950 dark:text-neutral-100">
      <div className="mx-auto max-w-2xl px-6 py-10">
        <Link
          href="/dashboard"
          className="mb-6 inline-flex items-center gap-1 text-sm text-[#6B5D4F] transition-colors hover:text-[#2B2118] dark:text-neutral-500 dark:hover:text-neutral-300"
        >
          ← Dashboard
        </Link>

        <div className="mb-4 flex flex-wrap gap-2 font-mono text-xs text-[#6B5D4F] dark:text-neutral-500">
        ...
        <div
          className="text-base leading-relaxed text-[#2B2118] dark:text-neutral-100"
          dangerouslySetInnerHTML={{ __html: renderMathText(q.statement) }}
        />

        {q.diagramSvg && (
          <div
            className="my-6 flex justify-center rounded-lg border border-[#F0E6D6] bg-white p-4 dark:border-neutral-800"
        ...
        <div className="mt-8 border-t border-[#F0E6D6] pt-4 dark:border-neutral-800">
...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full border border-[#F0E6D6] bg-white px-2.5 py-1 dark:border-neutral-800 dark:bg-neutral-900">
      {children}
    </span>
  );
}