        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            <LeaderboardCard
              ...
              entries={topStreaks.map((u) => ({ id: u.id, name: displayName(u.name, u.email), value: `${u.currentStreak} day${u.currentStreak !== 1 ? "s" : ""}` }))}
            ...
            <LeaderboardCard
              ...
              entries={topRatings.map((u) => ({ id: u.id, name: displayName(u.name, u.email), value: `★ ${u.learnerScore}` }))}
        ...
        <div className="mt-5 flex flex-col gap-1.5">
          {entries.map((entry, i) => (
            <div key={entry.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-[#FFFBF2] dark:hover:bg-neutral-800">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ backgroundColor: i < 3 ? RANK_COLORS[i] : "#D8CBB5" }}>
                {i + 1}
              </span>
