  ...
  return (
    <div className="overflow-hidden bg-[#FFFBF2] dark:bg-neutral-950">
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute -bottom-10 -left-16 h-72 w-96 opacity-90">
          <Blob color="#FFDCC0" className="absolute inset-0 translate-x-3 translate-y-3" />
          <Blob color="#FF6B4A" className="absolute inset-0" />
        </div>
        ...
        <div className="relative mx-auto max-w-6xl px-6 pb-10">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            {EXAMS.map((exam) => (
              <span key={exam} className="rounded-full bg-[#ECE8FA] px-4 py-2 text-sm font-bold text-[#4C3AA0] dark:bg-indigo-950/40 dark:text-indigo-300">
                {exam}
              </span>
            ))}
          </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute -bottom-16 -left-20 h-80 w-96">
          <Blob color="#FF6B4A" className="opacity-90" />
        </div>
        <div className="pointer-events-none absolute -bottom-16 -right-20 h-80 w-96 -scale-x-100">
          <Blob color="#4C3AA0" className="opacity-90" />
        </div>
        <DotGrid color="#FF6B4A" className="pointer-events-none absolute left-8 top-1/2 hidden md:block" />
        <DotGrid color="#FFFFFF" className="pointer-events-none absolute bottom-16 right-16 hidden md:block" />
          ...
          <div className="mt-12 flex flex-col gap-5 md:flex-row md:items-stretch">
            {STEPS.map((step, i) => (
              <div key={step.num} className="flex flex-1 items-center">
                <div className="relative w-full rounded-2xl border border-[#F0E6D6] bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
                  <span
                ...
                {i < STEPS.length - 1 && (
                  <div className="hidden shrink-0 px-2 text-[#FF6B4A] md:block">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-16">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl border border-[#F0E6D6] bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
                <div className="relative flex h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: f.bg }}>
                  <f.icon size={22} color={f.fg} />
                  <MotionLines color={f.fg} className="absolute -top-2 -right-2" />
                </div>
